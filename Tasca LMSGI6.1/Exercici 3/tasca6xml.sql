DROP DATABASE IF EXISTS tasca6;

CREATE DATABASE IF NOT EXISTS tasca6;
...
INSERT INTO employees VALUES(1, '<?xml version="1.0" encoding="UTF-8"?>
<employees>
...
-- 3.2.1. Com podem obtenir els correus electrònics de tots els empleats que tenen el títol de treball "VP Sales"? 

SELECT ExtractValue(document, '//employee[jobTitle="VP Sales"]/email') 
FROM employees;

-- 3.2.2 Quants empleats treballen a la regió (territory) "EMEA"?)

SELECT ExtractValue(document, 'count(//employee[office/Territory="EMEA"])')
FROM employees;

-- 3.2.3 Quins són els cognoms dels empleats que tenen com a superior a l'empleat amb el codi  employeeNumber="1056"?

SELECT ExtractValue(document, '//employee[reportsTo=1056]/lastName')
FROM employees;

-- 3.2.4 Quina és la ciutat de l'oficina de l'empleat amb employeeNumber="1166"?
